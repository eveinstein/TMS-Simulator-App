...
// Model scale factor - must match TMSScene.jsx
// Models are ~10 units tall, scaled to ~0.3 units in world space
...
function PulseRing({ position, normal }) {
  ...
  return (
    <group position={position} rotation={rotation}>
      {rings.map(ring => (
        <mesh key={ring.id} position={[0, 0.05, 0]}>
          <ringGeometry args={[ring.scale, ring.scale + 0.2, 32]} />
          <meshBasicMaterial
            color="#63b3ed"
            transparent
...
export function TMSCoil() {
  ...
  const updateNearestTarget = useCallback((pos) => {
    ...
    // Convert to mm (model ~10 units tall ≈ 20cm, so 1 unit ≈ 20mm)
    const distMm = nearestDistance * 20;
    setNearestTarget(nearestName, distMm);
    
    return { name: nearestName, distance: nearestDistance };
  }, [targetPositions, setNearestTarget]);
  
  // Raycast from a model-space position toward head center to find scalp point
  // Returns intersection in model space
  const raycastToScalpFromPosition = useCallback((modelSpacePos) => {
    if (!headMesh) return null;
    
    // Get head center in local/geometry space
    ...
    const direction = modelSpacePos.clone().sub(headCenter).normalize();
    ...
    if (intersects.length > 0) {
      // Intersection point is in model space (geometry coordinates)
      return {
        point: intersects[0].point.clone(),
        normal: intersects[0].face.normal.clone().normalize(),
      };
    }
    return null;
  }, [headMesh]);
  ...
  // Returns intersection in model space (for use with coil positioning)
  const raycastToScalp = useCallback((screenX, screenY) => {
    ...
    if (intersects.length > 0) {
      // Intersection point is in world space (scaled by MODEL_SCALE)
      // Convert to model space by dividing by scale factor
      const worldPoint = intersects[0].point;
      const modelPoint = worldPoint.clone().divideScalar(MODEL_SCALE);
      
      // Normal needs to be transformed by world matrix and normalized
      // For uniform scale, direction is preserved
  ...
  useEffect(() => {
    if (isCoilLocked && lockedTarget && targetPositions[lockedTarget]) {
      ...
      // Use raycastToScalpFromPosition which works in model space
      const hit = raycastToScalpFromPosition(targetPos);
      if (hit) {
        // Position coil at target with normal offset (model space)
        const offset = hit.normal.clone().multiplyScalar(0.5);
  ...
  }, [isCoilLocked, lockedTarget, targetPositions, raycastToScalpFromPosition, setCoilPosition, setCoilNormal, setCoilRotation, calculateCoilOrientation, userYawOffset]);
